...
export class BulkDownloadService {
  ...
  static async downloadAsZip(
    ...
    try {
      ...
      const categorizedStickers = new Map<string, StickerForDownload[]>();
      
      stickers.forEach(sticker => {
        const category = sticker.category || 'Uncategorized';
        if (!categorizedStickers.has(category)) {
          categorizedStickers.set(category, []);
        }
        categorizedStickers.get(category)!.push(sticker);
      });

      // Download all stickers and add to ZIP
      const downloadPromises = stickers.map(async (sticker) => {
        try {
          ...
          const fileExtension = this.getFileExtension(sticker.imageUrl) || 'webp';
          const fileName = `${sticker.name.replace(/[^a-zA-Z0-9]/g, '_')}.${fileExtension}`;
          
          // Add to category folder
          const categoryFolder = sticker.category || 'Uncategorized';
      ...
      // Add info file if there were errors
      if (errors.length > 0) {
        const errorInfo = `Download Errors:\n${errors.join('\n')}`;
        zip.file('download_errors.txt', errorInfo);
      }

      // Add pack info file
      const packInfo = this.generatePackInfo(stickers, packName);
      zip.file('pack_info.txt', packInfo);
  ...
  private static generatePackInfo(stickers: StickerForDownload[], packName: string): string {
    const categories = [...new Set(stickers.map(s => s.category))];
    const totalSize = stickers.length;
    
    return `Sticker Pack: ${packName}
Created: ${new Date().toLocaleString()}
Total Stickers: ${totalSize}
Categories: ${categories.join(', ')}

Stickers:
${stickers.map((s, i) => `${i + 1}. ${s.name} (${s.category})`).join('\n')}

Generated by Funny Yellow
https://funnyyellow.com
`;
  }

  static validateSelection(stickers: StickerForDownload[], format: 'zip' | 'whatsapp-pack' | 'individual'): {
    isValid: boolean;
    error?: string;
  } {
    if (stickers.length === 0) {
    ...
    if (format === 'whatsapp-pack' && stickers.length > 30) {
      return { 
        isValid: false, 
        error: 'WhatsApp packs can contain maximum 30 stickers' 
      };
    }
